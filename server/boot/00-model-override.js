...
// to enable these logs set `DEBUG=boot:00-model-override` or `DEBUG=boot:*`
var log = require('debug')('boot:00-model-override');
...
module.exports = function (app, cb) {
  ...
  for(var model in app.models){
    app.models[model].getDataSource().setMaxListeners(0);
  }
  ...
  var aclHelper = new LoopbackModelHelper('ACL');
  aclHelper.findOrCreateMany(
    newAclEntries,
    newAclEntries,
    function (err, newAcls) {
      if (err) {
        log(err);
        cb();
        return;
